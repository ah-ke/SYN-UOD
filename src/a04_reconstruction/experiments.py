...
class Pix2PixHD_Generator:
	...
	def tr_generator(self, pred_labels_trainIds, **_):
		...
		labels = self.table_trainId_to_fullId_cuda[pred_labels_trainIds.reshape(-1).long()].reshape(pred_labels_trainIds.shape)  #  trainId → labelId
		labels_fakeErr_fullIds = labels
		...
		gen_image = (gen_out + 1) * 128  # 缩放到 [0, 256] 之间
		gen_image = torch.clamp(gen_image, min=0, max=255)
		gen_image = gen_image.type(torch.uint8)

		# import torchvision
		# torchvision.transforms.ToPILImage()(gen_image[0].cpu()).show()

		return dict(
			gen_image_raw = gen_out,
			gen_image = gen_image,
			labels_fakeErr_fullIds = labels_fakeErr_fullIds
		)

	# 0.2. generating image with label (labels_fakeErr_trainIds)
	def tr_generator_np(self, pred_labels_trainIds, **_):
		...
		# add
		labels_fakeErr_fullIds = out['labels_fakeErr_fullIds'][0].cpu().numpy()
