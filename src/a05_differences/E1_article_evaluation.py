...
# 在语义分割基础上执行异常检测（anomaly detection）评估
class EvaluationDetectingUnexpected:
	...
	# 生成并可视化模型推理过程中的示例图像，用于展示模型对图像中异常区域的检测效果
	...
	def run_roc_curves_for_variant(self, dset, anomaly_variants=None, on_road=False):
		...
		# anomaly_variants = anomaly_variants[:-2]

		roi_field = 'roi' if not on_road else 'roi_onroad'

		# Pipeline
		# (1) Load ground_truth labels
		tr_roc = TrsChain(
			TrChannelLoad('labels_source', 'labels_source'),
			dset.tr_get_anomaly_gt,
			dset.tr_get_roi,
		...
		# by doing all at once we only load the ground_truth once
...
# 作用是构建一个完整的异常检测流水线，将语义分割、生成模型和异常检测模型结合起来，
# 对输入图像进行分析，生成预测标签、生成图像和异常概率图，并保存结果。
