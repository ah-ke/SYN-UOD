...
class ExperimentDifference01(ExperimentBase):
	...
	def init_transforms(self):
		...
		# 定义预处理流程, TrsChain 是变换链（transformation chain），将多个预处理函数串联起来执行
	...
	# 从模型输出的 pred_anomaly_logits 中提取“属于异常类（label=1）”的概率图，通常用于生成最终的异常热图或用于后续阈值判断。
	...
	def tr_eval_batch_log(self, frame, fid, anomaly_p, **_):
		if fid in self.frames_to_log:
			...
			# ======== 保存热图图像（使用 MAGMA colormap）========
			cmap = cm.get_cmap('jet')
			normed = (frame.anomaly_p - frame.anomaly_p.min()) / (frame.anomaly_p.ptp() + 1e-8)
			pred_colorimg_heatmap = (cmap(normed)[:, :, :3] * 255).astype('uint8')
			imwrite(self.train_out_dir / f'e{epoch:03d}_anomalyP_heatmap_{fid_no_slash}.webp', pred_colorimg_heatmap)
			...
			# ========== 新增：保存原始异常分数到 HDF5 ==========
			anomaly_map_path = self.train_out_dir / 'anomaly_map.hdf5'
			with h5py.File(anomaly_map_path, 'a') as hdf:
				# 确保唯一键名（避免覆盖），支持分层命名
				key = str(fid) if isinstance(fid, str) else str(fid_no_slash)
				if key in hdf:
					del hdf[key]  # 若已存在，先删除
				hdf.create_dataset(key, data=frame.anomaly_p.astype('float16'))  # 存储为半精度
...
class ExperimentDifferenceBin_fakeErr(ExperimentDifference02_fakeErr):
	...
	def setup_dset(self, dset):
		dset.add_channels(
			pred_labels=channel_labels_fakeErr01,  # 存储模型生成的伪造标签
			gen_image=channel_reconstruction_trCTC_ssFakeErr,  # 存储重建图像
		)
		dset.tr_post_load_pre_cache.append(
			# accidentally saved fakeErr labels as sourceIds, so translate them to trainIds for consistency
			TrSemSegLabelTranslation(fields=['pred_labels'], table=CityscapesLabelInfo.table_label_to_trainId),
...
class ExperimentDifference_Auto_Base(ExperimentDifferenceBin_fakeErr):
	cfg = add_experiment(ExperimentDifferenceBin_fakePredErrBDD.cfg,
		...
		gen_img_ext = '.jpg',
	    pix2pix_variant = '0405_nostyle_crop_ctc',  # pix2pixHD checkpoint
		net=dict(
			batch_eval=3,
	...
	def init_discrepancy_dataset_channels(self):
		...
		self.ch_labelsfullFake = ChannelLoaderImage(dir_disrepancy_dset / 'labels' / '{dset.split}' / '{fid}_gtFine_labelIds.png') # add
	...
	def setup_dset(self, dset):
		...
		dset.tr_post_load_pre_cache = TrsChain()
		dset.set_channels_enabled('image', 'gen_image', 'semseg_errors')  # 仅处理指定的数据通道
		dset.discover()
	...
	def init_default_datasets(self, b_threaded=False):
		...
		dsets_ctc = [dset_ctc_train, dset_ctc_val]
		for dset in dsets_ctc:
			self.setup_dset(dset)  # 数据集的额外初始化

		# val集中观测数据设定
		self.frames_to_log = set([dset_ctc_val.frames[i].fid for i in [0, 1, 2, 3, 6, 8, 9]])
		# self.frames_to_log = set([dset_ctc_val.frames[i].fid for i in [80, 269, 500, 501]])

		# for i, frame in enumerate(dset_ctc_val.frames):
		# 	if frame.fid.endswith('ulm_000075_000019'):
	...
	def discrepancy_dataset_init_pipeline(self, use_gt_labels=True, write_orig_label=False):
		...
		self.tr_alter_labels_and_gen_image = TrsChain(
			...
			self.synthetic_mod,
			# synthetize image
			bind(self.pix2pix.tr_generator_np, pred_labels_trainIds='labels_fakeErr_trainIds').outs(gen_image='gen_image', labels_fakeErr_fullIds='labels_fakeErr_fullIds'),
		)
		...
		self.tr_synthetic_and_save = TrsChain(
			...
			TrByField('semseg_errors', lambda x: (x > 0).astype(np.uint8)*255),  # 1.
			# write to disk
			TrChannelSave(self.ch_discrepancy_mask, 'semseg_errors'),  # 2.
			TrChannelSave(self.ch_labelsFake, 'labels_fakeErr_trainIds'),  # 3.
			TrChannelSave(self.ch_labelsfullFake, 'labels_fakeErr_fullIds'),  # 4. (add)
			TrChannelSave(self.ch_reconstruction, 'gen_image'), # 5.

			# fake labels colorimg
			TrColorimg('labels_fakeErr_trainIds'), # 6.
			TrChannelSave(self.ch_labelsFake_colorimg, 'labels_fakeErr_trainIds_colorimg'), # 7.
		)
	...
	def discrepancy_dataset_generate(self, dsets=None, b_show=False, write_orig_label=False):
		...
		for dset in dsets:
			...
			else: # 对整个数据集调用 Frame.frame_list_apply，用 self.tr_synthetic_and_save 处理（真正批量生成并保存）
...
class Exp0521_SwapFgd_ImgAndLabelsVsGen_semGT(Exp0520_Diff_ImgAndLabelsVsGen_semGT):
	cfg = add_experiment(Exp0520_Diff_ImgAndLabelsVsGen_semGT.cfg,
		...
		gen_name='051X_semGT_fakeSwapFgd_genNoSty',
		swap_fraction=0.5,
    )

	def init_transforms(self):
		super().init_transforms()
		self.synthetic_mod = partial(tr_synthetic_swapFgd_labels, swap_fraction = self.cfg['swap_fraction'])  # partial函数将 tr_synthetic_swapFgd_labels 中的 swap_fraction 参数固定, 创建了新的函数
