	...
	def __init__(self, dir_root=DIR_CITYSCAPES, split='train', b_cache=True):
		...
		self.add_channels(
			image=ChannelLoaderImage(
				file_path_tmpl=f'{self.dir_root}/leftImg8bit/{self.split}/{{fid}}_leftImg8bit{{channel.img_ext}}',
			),
			labels_source=ChannelLoaderImage(
				img_ext='.png',
				file_path_tmpl=f'{self.dir_root}/gtFine/{self.split}/{{fid}}_gtFine_labelIds{{channel.img_ext}}',
			),
			instances=ChannelLoaderImage(
				img_ext='.png',
				file_path_tmpl=f'{self.dir_root}/gtFine/{self.split}/{{fid}}_gtFine_instanceIds{{channel.img_ext}}',
			),
		)
		# 移除指定的元素 'instances'
		self.channel_disable('instances')
		# 将原始标签 labelSource 转换为训练标签 ID（trainId=19）
