...
class DatasetBase(PytorchDatasetInterface):
	...
	# 用于设置哪些通道（channels）在后续的处理或操作中被启用
...
class ChannelLoaderImage(ChannelLoaderFileCollection):
    def __init__(self, file_path_tmpl=None, dir_root=None, suffix='', img_ext='.jpg'):
        ...
        """
        if file_path_tmpl is None:
            file_path_tmpl = self.file_path_tmpl_generic  # 确保默认使用自定义解析方法
    ...
    def file_path_tmpl_generic(channel, dset, fid):
        """ 使用 dir_root 直接构造路径，避免 dset 解析问题 """
        dir_root = getattr(channel, 'dir_root', None) or getattr(dset, 'dir_root', '')
        suffix = getattr(channel, 'suffix', '')
        img_ext = getattr(channel, 'img_ext', '.jpg')

        return str(Path(dir_root) / f"{fid}{suffix}{img_ext}")
