...
from torch.utils.data._utils.collate import string_classes, np_str_obj_array_pattern
...
def default_collate_edited(batch):
	...
	if isinstance(elem, torch.Tensor):
		...
		if elem_type.__name__ == 'ndarray':
			...
			return default_collate_edited([torch.as_tensor(np.copy(b)) for b in batch])
...
class Pipeline:
	...
	def apply_tr_output(self, frame):
		...
		return frame
	# 该方法从数据集中逐批获取数据，执行定义好的处理流程（如前向传播、损失计算、输出提取），可选地并发执行后处理，并将结果累积返回。
		...
		with ThreadPoolExecutor(max_workers=max(self.loader_args['batch_size'], 1)) as pool:
			with torch.set_grad_enabled(b_grad):
				for loader_item in loader:
					...
					batch.apply(self.tr_batch)  # 处理链（比如模型前向 + 损失计算）
