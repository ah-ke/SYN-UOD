...
class ExperimentBase():
	...
	def init_net(self, role):
		...
		# 	# 计算模型大小（MB）
		# 	# -----------------------------
		# 	def get_model_stats(model, model_name="Model"):
		# 		param_count = sum(p.numel() for p in model.parameters())
		# 		buffer_count = sum(b.numel() for b in model.buffers())
		# 		size_MB = (param_count + buffer_count) * model.parameters().__next__().element_size() / (1024 ** 2)
		# 		print(f"{model_name}: 参数量 = {param_count}, 模型大小 = {size_MB:.2f} MB")
		# 		return param_count, size_MB
		#
		# 	param_count, model_size_MB = get_model_stats(self.net_mod, model_name="net_mod")
		# 	print(f"net_mod: 参数量 = {param_count}, 模型大小 = {model_size_MB:.2f} MB")
	...
	def val_epoch(self, epoch_idx):
		self.val_epoch_start(epoch_idx)  # self.net_mod.eval()
	...
	def construct_default_pipeline(self, role):
		...
		# 构建验证阶段的两条处理链条 tr_batch 和 tr_output，分别用于模型前向传播及损失计算，以及后处理和清理。
		elif role == 'val':
			tr_batch = TrsChain([
				TrCUDA(),
				self.net_mod,
